...
let AdsRepo = class AdsRepo {
    ...
    async create(ad) {
        const result = await this.db.query("INSERT INTO ads (title, description, imageUrl, buttonText, buttonUrl, format, pages, isActive) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [ad.title, ad.description, ad.imageUrl, ad.buttonText, ad.buttonUrl, ad.format, JSON.stringify(ad.pages), ad.isActive !== false]);
